            ...
            issue_preview=[
                {
                    ...
                    "message": item.get("message") or item.get("category") or "待核问题",
                    "kks_code": item.get("kks_code", ""),
                    "name": item.get("name", ""),
...
['dragleave','drop'].forEach(type=>uploadZone.addEventListener(type,e=>{e.preventDefault();uploadZone.classList.remove('dragging')}));
 ...
 function renderAuditSummary(d){const ai=d.ai_review||{};if(d.issue_count!==undefined)setText('issueCount',d.issue_count);if(d.resolved_count!==undefined)setText('resolvedCount',d.resolved_count);if(ai.candidate_count!==undefined){setText('aiCount',ai.candidate_count||0);setText('aiReviewed',ai.reviewed_count||0);setText('aiMetricNote',ai.status==='completed'?'已完成复核':'等待模型复核')}}
 function renderDistribution(counts,total){const p0=Number(counts.P0||0),p1=Number(counts.P1||0),p2=Number(counts.P2||0),sum=p0+p1+p2,totalValue=Number(total)||sum||1,a=p0/totalValue*100,b=p1/totalValue*100,c=p2/totalValue*100;setText('legendP0',p0);setText('legendP1',p1);setText('legendP2',p2);setText('priorityP0Table',p0);setText('priorityP1Table',p1);setText('priorityP2Table',p2);setText('priorityTotal',sum);setText('donutTotal',total||sum);setText('issueCount',total||sum);const bars=[['barP0',a],['barP1',b],['barP2',c]];bars.forEach(([id,value])=>{const el=document.getElementById(id);if(el)el.style.width=value.toFixed(2)+'%'});const donut=document.getElementById('issueDonut');if(donut)donut.style.background='conic-gradient(#ef4444 0 '+a+'%,#f59e0b '+a+'% '+(a+b)+'%,#2563eb '+(a+b)+'% 100%)'}
 function appendPreviewCell(row,value,className){const cell=document.createElement('td');cell.textContent=value||'—';if(className)cell.className=className;row.appendChild(cell)}
 function renderIssuePreview(items){const body=document.getElementById('issuePreview');body.innerHTML='';if(!Array.isArray(items)||!items.length){const row=document.createElement('tr');const cell=document.createElement('td');cell.colSpan=5;cell.className='empty-preview';cell.textContent='本次没有可展示的问题';row.appendChild(cell);body.appendChild(row);return}items.slice(0,5).forEach(item=>{const row=document.createElement('tr'),priority=String(item.priority||'P2'),status=item.ai_decision?'AI 已复核':item.status==='needs_review'?'AI 待复核':'待确认';appendPreviewCell(row,priority,'priority-cell '+priority.toLowerCase());appendPreviewCell(row,item.message||item.category||'待核问题');appendPreviewCell(row,item.kks_code||item.name||'—');appendPreviewCell(row,item.excel_row?'第'+item.excel_row+'行':'—');appendPreviewCell(row,status,'preview-status') ;body.appendChild(row)})}
 function renderCompletedResult(d){const ai=d.ai_review||{},counts=d.priority_counts||{},candidate=Number(ai.candidate_count||0),reviewed=Number(ai.reviewed_count||0);document.getElementById('result').hidden=true;document.getElementById('progressPanel').hidden=true;document.getElementById('resultDashboard').hidden=false;setText('uploadState','审核完成');setText('aiMetricNote',ai.status==='completed'?'已完成复核':ai.status==='error'?'AI 复核失败':'未启用 AI');setText('validRate','已读取有效编码');setText('aiCount',candidate?candidate:0);setText('aiReviewed',reviewed);renderDistribution(counts,d.issue_count);renderIssuePreview(d.issue_preview||[]);links.innerHTML='';for(const [label,url] of Object.entries(d.files||{})){const a=document.createElement('a');a.className='download-card';a.href=url;a.download='';const icon=document.createElement('span');icon.className='download-icon';icon.textContent=label.includes('HTML')?'</>':'XLSX';const copy=document.createElement('span');copy.className='download-copy';const title=document.createElement('strong');title.textContent=label;const desc=document.createElement('span');desc.textContent=label.includes('HTML')?'详细的审核报告，包含问题描述和建议':'完整的问题清单，支持筛选和排序';copy.append(title,desc);const action=document.createElement('span');action.className='download-action';action.textContent='下载';a.append(icon,copy,action);links.appendChild(a)}links.hidden=!Object.keys(d.files||{}).length;document.getElementById('viewAllIssues').onclick=()=>{const target=[...links.querySelectorAll('a')].find(a=>a.textContent.includes('XLSX'));if(target)target.click()}}
