...
def admin_token() -> str:
    """返回服务端配置的管理令牌；未配置时为空串（此时管理接口一律拒绝）。"""
    return os.environ.get(ADMIN_TOKEN_ENV, "").strip()


def _provided_token(headers: Any, raw_path: str) -> str:
    """从 Authorization: Bearer/Basic 或 ?token= 查询串中提取令牌。"""
    ...
    if lowered.startswith("basic "):
        try:
            decoded = base64.b64decode(header[6:].strip()).decode("utf-8", "ignore")
        except (ValueError, TypeError):
            return ""
        # Basic 约定 user:password，这里只认冒号后的口令部分
        return decoded.split(":", 1)[1].strip() if ":" in decoded else decoded.strip()
    return (parse_qs(urlparse(raw_path).query).get("token", [""])[0] or "").strip()
...
function isAuthErr(err){return Boolean(err&&(err.code==='unauthorized'||err.code==='admin_token_unset'))}
function authHint(err){return err&&err.code==='admin_token_unset'?'服务端未配置 KKS_ADMIN_TOKEN，请管理员在 .env 中设置后重启服务':'请先在「AI 配置」页填写管理令牌'}
async function jsonFetch(url,options={}){const opts=Object.assign({},options);const tk=adminTokenValue();if(tk){opts.headers=Object.assign({},options.headers||{},{Authorization:'Bearer '+tk})}const r=await fetch(url,opts);const d=await r.json().catch(()=>({}));if(!r.ok){const e=new Error(d.error||'请求失败');e.code=d.code;e.status=r.status;throw e}return d}
 const pageMeta={auditPage:['KKS QUALITY CONTROL','编码审核工作台','上传 Excel 文件，AI 智能识别并生成审核结果'],skillPage:['RULE MANAGEMENT','Skill 规则管理','维护 AI 参考文档和可执行审核规则'],configPage:['AI ADMINISTRATION','AI 配置','管理模型接口、密钥和审核参数'],logsPage:['SYSTEM ACTIVITY','运行日志','查看审核任务、配置变更和服务运行记录'],historyPage:['AUDIT HISTORY','审计台账','跨文件累计记录与汇总对比']};
function setPageMeta(page){const meta=pageMeta[page]||pageMeta.auditPage;document.getElementById('pageEyebrow').textContent=meta[0];document.getElementById('pageTitle').textContent=meta[1];document.getElementById('pageSubtitle').textContent=meta[2]}
 function activatePage(page){document.querySelectorAll('.nav-link').forEach(x=>x.classList.toggle('active',x.dataset.panel===page));document.querySelectorAll('.page').forEach(x=>x.classList.toggle('active',x.id===page));setPageMeta(page);if(page==='logsPage')loadLogs();if(page==='configPage')loadConfig();if(page==='historyPage')loadHistory()}
...
async function loadLogs(){const box=document.getElementById('logs'),button=document.getElementById('refreshLogs'),meta=document.getElementById('logMeta');setButtonBusy(button,true,'刷新中…','刷新日志');box.className='log-box';box.textContent='正在读取日志……';meta.textContent='正在刷新';try{const d=await jsonFetch('/api/logs?limit=300'),lines=Array.isArray(d.lines)?d.lines:[];box.textContent=lines.join('\\n')||'暂无运行日志';box.className=lines.length?'log-box':'log-box empty-state';meta.textContent='最近刷新：'+new Date().toLocaleTimeString()}catch(err){box.textContent=isAuthErr(err)?authHint(err):'读取日志失败：'+err.message;box.className='log-box empty-state';meta.textContent='刷新失败'}finally{setButtonBusy(button,false,'刷新中…','刷新日志')}}document.getElementById('refreshLogs').onclick=loadLogs;
(function(){var input=document.getElementById('adminToken'),btn=document.getElementById('saveAdminToken');if(!input||!btn)return;input.value=adminTokenValue();btn.onclick=async function(){try{localStorage.setItem('kksAdminToken',input.value.trim())}catch(e){}btn.textContent='已保存';setTimeout(function(){btn.textContent='保存令牌'},1500);await loadConfig();await loadSkill();await loadLogs();loadState()}})();
loadState();loadConfig();loadSkill();loadLogs();
</script><style>
...
function loadHistory(){
  var host=document.getElementById('historyHost');if(!host)return;
  ...
  fetch('/api/history').then(function(r){return r.json()}).then(function(d){
    ...
    if(dlL)dlL.style.display=entries.length?'':'none';
    if(!entries.length){host.innerHTML='<div class="muted" style="padding:10px 0">暂无审核记录，完成一次上传审核后自动生成。</div>';return}
    ...
    var kpis='<div class="biz-kpis">'
      +'<div class="biz-kpi"><b>'+entries.length+'</b><span>累计审核文件</span></div>'
      ...
      +'<div class="biz-kpi"><b>'+ta.toLocaleString()+'</b><span>累计 AI 复核</span></div></div>';
...
class AuditHandler(BaseHTTPRequestHandler):
    ...
    def require_admin(self) -> bool:
        """管理接口鉴权；未通过时响应已写好，调用方直接 return。

        - 服务端未配置 KKS_ADMIN_TOKEN：一律 403（fail-closed，避免默认裸奔）；
        - 令牌缺失或不匹配：401。比较使用常量时间，避免时序侧信道。
        """
        expected = admin_token()
        if not expected:
            LOGGER.warning("admin_api_blocked reason=token_unset path=%s", urlparse(self.path).path)
            self.send_json({
                "error": "管理接口已锁定：服务端未配置 KKS_ADMIN_TOKEN，请在 .env 中设置该变量并重启服务",
                "code": "admin_token_unset",
            }, 403)
            return False
        provided = _provided_token(self.headers, self.path)
        if not provided or not hmac.compare_digest(provided, expected):
            LOGGER.warning("admin_api_denied path=%s", urlparse(self.path).path)
            self.send_json({"error": "未授权：管理令牌缺失或无效", "code": "unauthorized"}, 401)
            return False
        return True
    ...
    def do_GET(self) -> None:
        ...
        if path == "/healthz":
            ...
            return
        if path in PROTECTED_GET and not self.require_admin():
            return
        if path == "/api/config":
        ...
        if path == "/api/history/download":
            from audit_history import HISTORY_NAME, LEDGER_NAME
            query = parse_qs(parsed.query)
            kind = (query.get("type", ["summary"])[0] or "summary").strip()
            if kind == "ledger":
                name, content_type = LEDGER_NAME, "application/json; charset=utf-8"
            else:
                name, content_type = HISTORY_NAME, "text/html; charset=utf-8"
            target = (self.runs_dir / name).resolve()
            if not target.is_file():
                self.send_json({"error": "台账文件尚未生成，请先完成一次审核"}, 404)
                return
            self.send_bytes(target.read_bytes(), content_type, download_name=name)
            return
        if path == "/api/logs":
            query = parse_qs(parsed.query)
            try:
    ...
    def do_POST(self) -> None:
        ...
        if path in PROTECTED_POST and not self.require_admin():
            return
        if path == "/api/config":
            try:
                self.handle_config()
