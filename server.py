...
['dragleave','drop'].forEach(type=>uploadZone.addEventListener(type,e=>{e.preventDefault();uploadZone.classList.remove('dragging')}));
 ...
 function setResultState(label,kind){const state=document.getElementById('resultState');state.textContent=label;state.className='result-state '+kind}
...
UPLOAD_PAGE = UPLOAD_PAGE.replace("</head>", RESULT_UI_STYLE + "</head>")
