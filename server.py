...
<div class="panel"><div class="panel-head"><div><h3>审核结果</h3><div class="muted">审核过程中显示规则阶段和 AI 全部候选复核状态，完成后可下载完整证据。</div></div></div><div id="progressPanel" hidden style="margin:0 0 14px;padding:14px 16px;border:1px solid #c7d2fe;border-radius:12px;background:#fafaff"><div style="display:flex;justify-content:space-between;gap:12px;font-size:13px;color:#334155"><span id="progressText">准备审核</span><b id="progressPercent" style="color:#4f46e5">0%</b></div><div style="height:8px;margin-top:10px;border-radius:999px;background:#e0e7ff;overflow:hidden"><div id="progressBar" style="height:100%;width:0%;border-radius:999px;background:#4f46e5;transition:width .45s ease"></div></div><div id="progressHint" style="margin-top:8px;font-size:12px;color:#64748b">正在准备文件……</div></div><div id="result" class="result-box">等待上传文件……</div><div id="links" class="links"></div></div></section>
...
uploadZone.addEventListener('drop',e=>{const file=e.dataTransfer.files[0];if(!file)return;if(!/\\.xlsx$/i.test(file.name)){showSelectedFile(null);result.textContent='请选择 .xlsx 格式的 Excel 文件';return}const transfer=new DataTransfer();transfer.items.add(file);fileInput.files=transfer.files;showSelectedFile(file)});
 ...
 function setProgress(value,text,hint){document.getElementById('progressBar').style.width=value+'%';document.getElementById('progressPercent').textContent=value+'%';document.getElementById('progressText').textContent=text;document.getElementById('progressHint').textContent=hint}
 function startProgress(){document.getElementById('progressPanel').hidden=false;setProgress(5,'审核任务已启动','已接收文件，正在等待后台任务开始')}
 function finishProgress(ok){setProgress(ok?100:0,ok?'审核完成':'审核未完成',ok?'报告已生成，可以下载结果':'请根据错误信息修正后重试')}
 function renderAuditSummary(d){const ai=d.ai_review||{};if(d.issue_count!==undefined)document.getElementById('issueCount').textContent=d.issue_count;if(d.resolved_count!==undefined)document.getElementById('resolvedCount').textContent=d.resolved_count;if(ai.candidate_count!==undefined){document.getElementById('aiCount').textContent=ai.candidate_count||0;document.getElementById('aiReviewed').textContent=ai.reviewed_count||0}}
 async function watchAudit(runId){while(true){const d=await jsonFetch('/api/audits/'+encodeURIComponent(runId)+'/status'),ai=d.ai_review||{};if(d.percent!==undefined)setProgress(d.percent,d.message||'正在审核',d.hint||'');renderAuditSummary(d);if(d.status==='completed'){result.textContent=(d.conclusion||'审核完成')+String.fromCharCode(10)+'有效编码：'+(d.data_rows||0)+String.fromCharCode(10)+'问题总数：'+(d.issue_count||0)+String.fromCharCode(10)+'AI状态：'+(ai.status||'disabled');links.textContent='';for(const [label,url] of Object.entries(d.files||{})){const a=document.createElement('a');a.href=url;a.textContent=label;a.download='';links.appendChild(a)}finishProgress(true);return}if(d.status==='error')throw new Error(d.hint||d.error||'审核失败');result.textContent=(d.message||'正在审核，请稍候……')+String.fromCharCode(10)+(d.hint||'');await new Promise(resolve=>setTimeout(resolve,600))}}
async function loadState(){try{const d=await jsonFetch('/healthz');setAiBadge(Boolean(d.ai_configured))}catch(err){setAiBadge(false)}}
 form.addEventListener('submit',async e=>{e.preventDefault();submit.disabled=true;submit.classList.add('is-loading');submit.textContent='审核准备中…';result.textContent='正在创建审核任务……';links.textContent='';document.getElementById('issueCount').textContent='—';document.getElementById('resolvedCount').textContent='—';document.getElementById('aiCount').textContent='—';document.getElementById('aiReviewed').textContent='—';startProgress();try{const created=await jsonFetch('/api/audits',{method:'POST',body:new FormData(form)});await watchAudit(created.run_id)}catch(err){result.textContent='审核失败：'+err.message;finishProgress(false)}finally{submit.disabled=false;submit.classList.remove('is-loading');submit.textContent='开始审核'}});
let configSnapshot='';
