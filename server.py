...
['dragleave','drop'].forEach(type=>uploadZone.addEventListener(type,e=>{e.preventDefault();uploadZone.classList.remove('dragging')}));
 ...
 function renderIssuePreview(items){const body=document.getElementById('issuePreview');body.innerHTML='';if(!Array.isArray(items)||!items.length){const row=document.createElement('tr');const cell=document.createElement('td');cell.colSpan=6;cell.className='empty-preview';cell.textContent='本次没有可展示的问题';row.appendChild(cell);body.appendChild(row);return}items.slice(0,5).forEach(item=>{const row=document.createElement('tr'),priority=String(item.priority||'P2'),status=item.ai_decision?'AI 已复核':item.status==='needs_review'?'AI 待复核':'待确认';appendPreviewCell(row,priority,'priority-cell '+priority.toLowerCase());appendPreviewCell(row,item.definition||item.category||'','def-cell');appendPreviewCell(row,item.message||item.category||'待核问题');appendPreviewCell(row,item.kks_code||item.name||'—');appendPreviewCell(row,item.excel_row?'第'+item.excel_row+'行':'—');appendPreviewCell(row,status,'preview-status') ;body.appendChild(row)})}
