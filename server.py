...
from run_audit import audit_file, output_artifact_names
...
def _apply_progress_event(server: ThreadingHTTPServer, run_id: str, event: dict[str, object]) -> None:
    ...
    if event.get("phase") == "ai":
        for key in ("status", "model", "candidate_count", "reviewed_count"):
            if key in event:
...
def _run_audit_job(
    ...
    try:
        _update_job_state(server, run_id, status="running", phase="starting", percent=5, message="审核任务已启动", hint="准备读取 Excel")
        result = audit_file(
            input_path,
            run_dir,
...
<aside class="sidebar"><div class="brand"><div class="brand-mark">K</div><div><strong>KKS 审核工作台</strong><small>编码质量控制中心</small></div></div><div class="nav-label">工作台</div><button class="nav-link active" data-panel="auditPage"><span>01</span>开始审核</button><button class="nav-link" data-panel="skillPage"><span>02</span>Skill 规则</button><button class="nav-link" data-panel="configPage"><span>03</span>AI 配置</button><button class="nav-link" data-panel="logsPage"><span>04</span>运行日志</button><div class="side-note">规则审查 · 证据留痕<br>源表只读 · AI 辅助复核</div></aside>
<main class="content"><header class="topbar"><div class="topbar-copy"><div id="pageEyebrow" class="eyebrow">KKS QUALITY CONTROL</div><h1 id="pageTitle">编码审核工作台</h1><div id="pageSubtitle" class="topbar-subtitle">上传 Excel，快速获得可追溯的审核结论</div></div><div id="aiBadge" class="pill">正在读取 AI 状态</div></header>
<section id="auditPage" class="page active">
<div class="audit-intro"><div><div class="section-kicker">AUDIT WORKSPACE</div><h2>提交一次编码审核</h2><p>上传编码人员的 Excel，系统先检查规则，再复核需要确认的语义问题。</p></div><span class="audit-source-badge">源 Excel 只读</span></div>
<div class="audit-input-grid"><div class="panel upload-panel"><div class="panel-head"><div><h3>选择审核文件</h3><div class="muted">支持 .xlsx；审核完成后只输出需要跟进的问题和证据。</div></div></div><form id="form" enctype="multipart/form-data"><div id="uploadZone" class="upload-zone compact"><div class="upload-icon">↑</div><div class="upload-title">拖拽 Excel 文件到这里</div><div class="upload-desc">或选择本地文件进行审核</div><label for="file" class="btn btn-secondary upload-select">选择 Excel 文件</label><input id="file" class="file-input" name="file" type="file" accept=".xlsx" required><div id="fileName" class="file-name">尚未选择文件</div><div class="upload-drop-hint">原始文件不会被修改</div><div class="upload-actions"><button id="submit" class="btn btn-primary" type="submit">开始审核</button></div></div></form></div><div class="panel audit-guide"><div class="audit-guide-header"><h3>审核流程</h3><span class="guide-caption">自动执行</span></div><div class="workflow-list"><div class="workflow-item"><span class="workflow-number">01</span><div><strong>规则检查</strong><span>识别结构、重复、命名和父子关系问题</span></div></div><div class="workflow-item"><span class="workflow-number">02</span><div><strong>AI 语义复核</strong><span>只处理规则识别出的不确定候选项</span></div></div><div class="workflow-item"><span class="workflow-number">03</span><div><strong>生成问题清单</strong><span>保留问题等级、证据和修改建议</span></div></div></div><div class="audit-guide-note">审核完成后，优先查看 P0 阻断项，再下载问题 Excel 转交编码人员。</div></div></div>
<div class="audit-section-bar"><div><h3>本次审核概览</h3><p>审核过程中自动更新，完成后显示最终数量。</p></div><span id="overviewState" class="overview-state">待上传</span></div><div class="summary-grid"><div class="metric issue"><small>问题总数</small><b id="issueCount">—</b></div><div class="metric"><small>已澄清项</small><b id="resolvedCount">—</b></div><div class="metric"><small>AI 复核候选</small><b id="aiCount">—</b></div><div class="metric"><small>AI 已复核</small><b id="aiReviewed">—</b></div></div>
<div id="resultPanel" class="panel result-panel"><div class="panel-head result-panel-head"><div><div class="section-kicker">RESULT / EVIDENCE</div><h3>审核结果</h3><div class="muted">先看审核结论和严重度，再下载 HTML 报告或问题 Excel。</div></div><span id="resultState" class="result-state idle">等待审核</span></div><div id="progressPanel" class="progress-panel" hidden><div class="progress-topline"><span id="progressText">准备审核</span><b id="progressPercent">0%</b></div><div class="progress-track"><div id="progressBar"></div></div><div id="progressHint" class="progress-hint">正在准备文件……</div></div><div id="result" class="result-box result-box-empty"><div class="result-message"><span class="result-message-icon" aria-hidden="true">◎</span><div><strong id="resultTitle">等待上传文件</strong><span id="resultSubtitle">审核完成后，结果和下载入口会显示在这里。</span></div></div></div><div id="resultStats" class="result-stats" hidden><div class="result-stat"><small>有效编码</small><b id="resultDataRows">—</b></div><div class="result-stat"><small>问题总数</small><b id="resultIssueCount">—</b></div><div class="result-stat"><small>AI 复核</small><b id="resultAiCount">—</b><span id="resultAiHint" class="result-stat-hint">—</span></div><div class="result-stat result-priority"><small>问题级别</small><div><span class="priority-chip p0">P0 <b id="priorityP0">0</b></span><span class="priority-chip p1">P1 <b id="priorityP1">0</b></span><span class="priority-chip p2">P2 <b id="priorityP2">0</b></span></div></div></div><div id="links" class="links result-downloads" hidden></div></div></section>
...
const pageMeta={auditPage:['KKS QUALITY CONTROL','编码审核工作台','上传 Excel，快速获得可追溯的审核结论'],skillPage:['RULE MANAGEMENT','Skill 规则管理','维护 AI 参考文档和可执行审核规则'],configPage:['AI ADMINISTRATION','AI 配置','管理模型接口、密钥和审核参数'],logsPage:['SYSTEM ACTIVITY','运行日志','查看审核任务、配置变更和服务运行记录']};
...
document.querySelectorAll('.nav-link').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('.nav-link').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.page').forEach(x=>x.classList.remove('active'));btn.classList.add('active');document.getElementById(btn.dataset.panel).classList.add('active');setPageMeta(btn.dataset.panel);if(btn.dataset.panel==='logsPage')loadLogs();if(btn.dataset.panel==='configPage')loadConfig()}));setPageMeta('auditPage');
const fileInput=document.getElementById('file'),uploadZone=document.getElementById('uploadZone');
function showSelectedFile(file){document.getElementById('fileName').textContent=file?file.name:'尚未选择文件'}
...
['dragleave','drop'].forEach(type=>uploadZone.addEventListener(type,e=>{e.preventDefault();uploadZone.classList.remove('dragging')}));
 ...
 function setAiBadge(ready){const badge=document.getElementById('aiBadge');badge.textContent=ready?'AI 已由管理员配置':'AI 尚未配置';badge.className='pill'+(ready?' ready':'')}
 ...
 function startProgress(){document.getElementById('progressPanel').hidden=false;document.getElementById('resultStats').hidden=true;links.hidden=true;setResultMessage('正在创建审核任务','文件已接收，正在执行规则检查和 AI 复核。','running');setResultState('排队中','running');setProgress(5,'审核任务已启动','已接收文件，正在等待后台任务开始')}
 function finishProgress(ok){setProgress(ok?100:0,ok?'审核完成':'审核未完成',ok?'报告已生成，可以下载结果':'请根据错误信息修正后重试');setResultState(ok?'已完成':'未完成',ok?'done':'failed');if(ok){window.setTimeout(()=>document.getElementById('resultPanel').scrollIntoView({behavior:'smooth',block:'start'}),80)}}
 function renderAuditSummary(d){const ai=d.ai_review||{};if(d.issue_count!==undefined)setText('issueCount',d.issue_count);if(d.resolved_count!==undefined)setText('resolvedCount',d.resolved_count);if(ai.candidate_count!==undefined){setText('aiCount',ai.candidate_count||0);setText('aiReviewed',ai.reviewed_count||0)}}
 function renderCompletedResult(d){const ai=d.ai_review||{},counts=d.priority_counts||{},candidate=Number(ai.candidate_count||0),reviewed=Number(ai.reviewed_count||0);setResultMessage(d.conclusion||'审核完成','源 Excel 未修改；请打开问题清单，按问题级别逐项核对。','complete');document.getElementById('resultStats').hidden=false;setText('resultDataRows',d.data_rows||0);setText('resultIssueCount',d.issue_count||0);setText('resultAiCount',candidate?reviewed+' / '+candidate:(ai.status==='disabled'?'未启用':'0'));setText('resultAiHint',ai.status==='disabled'?'AI 未启用':'已复核 / 候选');setText('priorityP0',counts.P0||0);setText('priorityP1',counts.P1||0);setText('priorityP2',counts.P2||0);links.innerHTML='';for(const [label,url] of Object.entries(d.files||{})){const a=document.createElement('a');a.className='download-card';a.href=url;a.download='';const icon=document.createElement('span');icon.className='download-icon';icon.textContent=label.includes('HTML')?'HTML':'XLSX';const copy=document.createElement('span');copy.className='download-copy';const title=document.createElement('strong');title.textContent=label;const desc=document.createElement('span');desc.textContent=label.includes('HTML')?'浏览器查看完整审核报告':'下载可转交的问题清单';copy.append(title,desc);const action=document.createElement('span');action.className='download-action';action.textContent='下载 →';a.append(icon,copy,action);links.appendChild(a)}links.hidden=!Object.keys(d.files||{}).length}
...
async function loadState(){try{const d=await jsonFetch('/healthz');setAiBadge(Boolean(d.ai_configured))}catch(err){setAiBadge(false)}}
  form.addEventListener('submit',async e=>{e.preventDefault();submit.disabled=true;submit.classList.add('is-loading');submit.textContent='审核准备中…';document.getElementById('issueCount').textContent='—';document.getElementById('resolvedCount').textContent='—';document.getElementById('aiCount').textContent='—';document.getElementById('aiReviewed').textContent='—';startProgress();try{const created=await jsonFetch('/api/audits',{method:'POST',body:new FormData(form)});await watchAudit(created.run_id)}catch(err){setResultMessage('审核失败',err.message||'请根据日志排查问题后重试。','failed');finishProgress(false)}finally{submit.disabled=false;submit.classList.remove('is-loading');submit.textContent='开始审核'}});
let configSnapshot='';
...
UPLOAD_PAGE = UPLOAD_PAGE.replace("</head>", RESULT_UI_STYLE + AUDIT_REDESIGN_STYLE + "</head>")
